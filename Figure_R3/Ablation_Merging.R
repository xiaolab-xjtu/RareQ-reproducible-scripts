...
FindRare_Ablation_Merging <- function(sc_object, assay='RNA', k=6, Q_cut=0.6, ratio=0.2, max_iter=100){
...
for(sc in scenarios){
  for(k in repeats){
    ...
    # cluster = FindRare_Ablation_Merging(sc_object = sc_object)
    # saveRDS(cluster, file=paste0(path, 'OUR_result_Ablation_Merging_1.RDS'))

    cluster = FindRare_Ablation_Merging(sc_object = sc_object, k=15)
    saveRDS(cluster, file=paste0(path, 'OUR_result_Ablation_Merging_k_15_1.RDS'))
