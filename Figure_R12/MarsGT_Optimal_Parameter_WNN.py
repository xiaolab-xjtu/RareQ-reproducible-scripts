...
parser.add_argument('--wd', type=float, default=0.3) # The 'weight_decay' parameter is used to specify the strength of L2 regularization
...
experiments = ["Human_retina_RNA_ATAC","Human_retina_rpe_choroid_RNA_ATAC","Mouse_gdT_RNA_ATAC","ISSAAC_mCortex_RNA_ATAC","Mouse_colon_RNA_ATAC","Chen_2019_RNA_ATAC","Mouse_Kidney_RNA_ATAC","Human_Gray_matter_RNA_ATAC","10x_Multiome_PBMC_Chromium_RNA_ATAC","10x_Multiome_Pbmc10k_RNA_ATAC"]
...
for experiment in experiments:
    file_path = path + experiment + '/pred_CPU_Parameter_Comparable.npy'
    #if(exists(file_path)):
    #  continue
    ...
    WNN_embed = ad.read_h5ad(path + experiment + '/WNN_embed.h5ad')
    ...
    resolution, _ = segment_function(RNA_matrix.shape[1])
    
    if __name__ == "__main__":
        device = torch.device("cuda" if cuda.is_available() else "cpu")
        print('You will use : ',device)
        # clustering result by scanpy
        initial_pre = initial_clustering(RNA_matrix, n_pcs=50, custom_n_neighbors=20, custom_resolution=resolution, use_rep=WNN_embed.X) 
        ...
        np.save(path + experiment + "/Node_Ids_CPU_Parameter_Comparable.npy", Node_Ids)
        np.save(path + experiment + "/pred_CPU_Parameter_Comparable.npy", MarsGT_result['pred_label'])
        np.save(path + experiment + "/cell_embedding_CPU_Parameter_Comparable.npy", MarsGT_result['cell_embedding'])
        
        del MarsGT_result, MarsGT_gnn, MarsGT_model, gene_cell, peak_cell, RNA_matrix, ATAC_matrix, Node_Ids, gnn, WNN_embed
    


for experiment in experiments:
    
    pred_file_path = path + experiment + '/pred_CPU_Parameter_Comparable.npy'
    Id_file_path = path + experiment + '/Node_Ids_CPU_Parameter_Comparable.npy'
    # Save numpy arrays to files
    pred = np.load(pred_file_path)
    pred_df = pd.DataFrame(pred)
    pred_df.to_csv(path + experiment + '/pred_CPU_Parameter_Comparable.csv', index=False, header=False)
    id = np.load(Id_file_path)
    id_df = pd.DataFrame(id)
    id_df.to_csv(path + experiment + '/Node_Ids_CPU_Parameter_Comparable.csv', index=False, header=False)
    del pred, pred_df, id, id_df
