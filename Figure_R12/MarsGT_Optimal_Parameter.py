...
for experiment in experiments:
    file_path = path + experiment + '/pred_CPU_Optimal_Parameter.npy'
    if(exists(file_path)):
      continue
    ...
    if __name__ == "__main__":
        ...
        np.save(path + experiment + "/Node_Ids_CPU_Optimal_Parameter.npy", Node_Ids)
        np.save(path + experiment + "/pred_CPU_Optimal_Parameter.npy", MarsGT_result['pred_label'])
        np.save(path + experiment + "/cell_embedding_CPU_Optimal_Parameter.npy", MarsGT_result['cell_embedding'])
        
        del MarsGT_result, MarsGT_gnn, MarsGT_model, gene_cell, peak_cell, RNA_matrix, ATAC_matrix, Node_Ids, gnn
...
for experiment in experiments:
    ...
    pred_file_path = path + experiment + '/pred_CPU_Optimal_Parameter.npy'
    Id_file_path = path + experiment + '/Node_Ids_CPU_Optimal_Parameter.npy'
    # Save numpy arrays to files
    pred = np.load(pred_file_path)
    pred_df = pd.DataFrame(pred)
    pred_df.to_csv(path + experiment + '/pred_CPU_Optimal_Parameter.csv', index=False, header=False)
    id = np.load(Id_file_path)
    id_df = pd.DataFrame(id)
    id_df.to_csv(path + experiment + '/Node_Ids_CPU_Optimal_Parameter.csv', index=False, header=False)
