...
Evaluate.parameter <- function(parameter.list, parameter.symbol){
  ...
  perf.df$para.value = factor(perf.df$para.value, levels=c(0,1), labels = c('Standard', 'Q_ablation'), ordered = T)

  saveRDS(perf.df, file = paste0('Figure/F1_Precision_Recall_20_datasets_Ablation_Q.RDS'))

}


ks <- c(0, 1) # 0 denote the unshuffled result
p.k <- Evaluate.parameter(parameter.list = ks, parameter.symbol = 'Ablation_Q')
...
Evaluate.parameter.NMI <- function(parameter.list, parameter.symbol){
  ...
  saveRDS(perf.df, file = paste0('Figure/NMI_20_datasets_Ablation_Q.RDS'))
