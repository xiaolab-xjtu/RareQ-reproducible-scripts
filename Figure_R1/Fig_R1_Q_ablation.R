...
colnames(df) <- c("Standard", "Ablation (Q)")

df_long <- df %>%
  select(Standard, `Ablation (Q)`) %>% 
  pivot_longer(
    cols = everything(),                 
    names_to = "Condition",              
    values_to = "F1"                    
...
df_long <- df_long %>%
  mutate(Condition = factor(Condition, levels = c("Standard", "Ablation (Q)")))
...
p1 <- ggplot(df_cdf, aes(x = cumulative, y = F1, color = Condition)) +
  ...
  scale_color_manual(values = c("Standard" = "#C72228", "Ablation (Q)" = "#3F84AA")) +
  theme1
p1
...
for (Ri in groups){
  ...
  colnames(df_i) <- c("Standard", "Ablation (Q)")
...
df <- df %>% 
  mutate(para.value = gsub("Q_ablation","Ablation (Q)",para.value))
df <- df %>%
  mutate(para.value = factor(para.value, levels = c("Standard", "Ablation (Q)")))
