...
## Choroid plexus compare
# Originally labelled as #151

cluster.df <- data.frame(X = seu$X,
                         Y = seu$Y,
                         Cluster = seu$Cluster,
                         cluster = seu$cluster_ordered)

library(ggplot2)
library(RColorBrewer)
library(ggpubr)

plot.cluster = function(cluster, label){

  cluster.df$group = ifelse(cluster.df[,cluster]==label, 'A', 'B')
  p.cluster <- ggplot(data = cluster.df) + geom_point(aes(x = X, y = Y, color=factor(group)), size=0.001) +
...
p.orig.CHP = plot.cluster(cluster='Cluster', label=151)


table(seu$cluster_ordered[seu$Cluster==151])


p.CHP.1 = plot.cluster(cluster='cluster', label=1)
p.CHP.2 = plot.cluster(cluster='cluster', label=11)

p.CHP.comb <- ggpubr::ggarrange(p.orig.CHP, p.CHP.1, p.CHP.2, ncol=1, align='hv')

X.range <- diff(range(cluster.df$X))
Y.range <- diff(range(cluster.df$Y))

ggsave(p.CHP.comb, filename = 'smFish_cluster_Choroid_plexus.pdf', width = X.range/(140), height = Y.range/(140) * 3, units = 'mm')


## Differentially expressed gene
Idents(seu) = 'cluster_ordered'
CHP.mk = FindMarkers(seu, ident.1 = 1, ident.2 = 11, only.pos = F, logfc.threshold=0, min.pct = 0)

CHP.obj = subset(seu, subset=cluster_ordered %in% c(1,11))

p.feature.CHP = DotPlot(CHP.obj, features = rownames(CHP.mk)[abs(CHP.mk$avg_log2FC) > 0.5], idents = c(1, 11), scale = F) + coord_flip() + labs(y='Choroid plexus clusters', x=NULL)
p.feature.CHP
ggsave(p.feature.CHP, filename = 'smFish_Choroid_Plexus_DotPlot_Feature.pdf', width = 4, height = 3)


## plot zoom-in position of Choroid plexus
cluster.df$Group1 =  ifelse(seu@assays$RNA@data['Foxj1',] >= 1 & seu@assays$RNA@data['Otx2',] >= 1, 1, 0)
cluster.df$Group11 =  ifelse(!(seu@assays$RNA@data['Foxj1',] >= 1 & seu@assays$RNA@data['Otx2',] >= 1) & cluster.df$cluster %in% c(1,11), 1, 0)

plot.cluster <- function(group, x.lim, y.lim){
  cluster.df$Group = cluster.df[,group]
  p.gene <- ggplot(data = cluster.df) + geom_point(aes(x = X, y = Y, color=factor(Group)), size=0.01) +
    theme_bw() + theme_minimal() +
    theme(panel.grid = element_blank(), legend.position = 'none',
          axis.text = element_blank(), panel.border = element_rect(colour = "black", fill=NA),
          axis.title = element_blank()) +
    scale_color_manual(values = c('grey80', 'red')) +
    scale_x_continuous(limits = x.lim, expand = c(0,0)) + scale_y_continuous(limits = y.lim, expand = c(0,0))
  p.gene
}


p.genes.CHP.cluster <- ggarrange(plot.cluster('Group1', x.lim = c(18000,26000), y.lim=c(13000,21000)),
                         plot.cluster('Group11', x.lim = c(18000,26000), y.lim=c(13000,21000)),
                         ncol=1, nrow=2)

ggsave(p.genes.CHP.cluster, filename = 'smFish_Choroid_plexus_cluster_by_Foxj1_Otx2.pdf', height = 90*1.25, width = 45*1.25, units = 'mm')
